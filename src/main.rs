...
fn string_to_list(string: &str) -> Vec<LE> {
    ...
    loop {
        ...
        match c {
            '[' => {
                ...
                let temp_list = string_to_list(
                    chars[index..index + offset]
                        .iter()
                        .collect::<String>()
                        .as_str(),
                );
                output.push(LE::List(temp_list));
                index += offset;
            }
            ']' => break,
            ',' => index += 1,
            _ => {
                let mut offset = 1;
                while chars[index + offset].is_numeric() {
                    offset += 1;
                }
                if let Ok(num) = chars[index..index + offset]
                    .iter()
                    .collect::<String>()
                    .as_str()
                    .parse::<i32>()
...
fn parse_input() -> Vec<(Vec<LE>, Vec<LE>)> {
    ...
    while let (Some(line_one), Some(line_two)) = (lines_iter.next(), lines_iter.next()) {
        let first = string_to_list(line_one);
        let second = string_to_list(line_two);
...
fn compare(packet_one: Vec<LE>, packet_two: Vec<LE>) -> Ordering {
    let len1 = packet_one.len();
    let len2 = packet_two.len();
    let size = if len1 < len2 { len1 } else { len2 };
    let smaller = len1.cmp(&len2);
    let mut i = 0;
    while i < size {
        let left = &packet_one[i];
        let right = &packet_two[i];
        match (left, right) {
            (LE::Num(n), LE::Num(m)) => match n.cmp(m) {
                Ordering::Less => return Ordering::Less,
                Ordering::Greater => return Ordering::Greater,
                Ordering::Equal => {}
            },
            (LE::Num(n), LE::List(y)) => {
                let cmp_result = compare(vec![LE::Num(*n)], y.to_vec());
                if cmp_result != Ordering::Equal {
                    return cmp_result;
                }
            }
            (LE::List(x), LE::Num(m)) => {
                let cmp_result = compare(x.to_vec(), vec![LE::Num(*m)]);
                if cmp_result != Ordering::Equal {
                    return cmp_result;
                }
            }
            (LE::List(x), LE::List(y)) => {
                let cmp_result = compare(x.to_vec(), y.to_vec());
                if cmp_result != Ordering::Equal {
                    return cmp_result;
                }
            }
...
fn main() {
    ...
    for (i, pair) in pairs.iter().enumerate() {
        let (left, right) = pair;
        let cmp_result = compare(left.to_vec(), right.to_vec());
